...
size_t run(std::shared_ptr<TChain> _chain, const std::shared_ptr<Histogram> &_hists, const std::shared_ptr<QA::QADB> &_qa = nullptr, int thread_id = 1)
{
        ...
        if (_qa)
        {
                _qa->CheckForDefect("TotalOutlier");    // bit 0
                _qa->CheckForDefect("TerminalOutlier"); // bit 1
                _qa->CheckForDefect("MarginalOutlier"); // bit 2
                _qa->CheckForDefect("SectorLoss");      // bit 3
                _qa->CheckForDefect("Misc");            // bit 5

                // Explicitly allow Misc for now (optional, but safe)
                //    _qa->AllowMiscBit(true);
        }
        for (size_t current_event = 0; current_event < num_of_events; current_event++)
        {
                ...
                const int run = data->getRun();
                const int evt = data->getEvent();

                // Apply Pass-2 QA cuts (only bits 0,1,4)
                ...
                if (!_qa->Pass(run, evt))
                {
                    std::cout << "[QADB FAIL] run=" << run
                              << " event=" << evt
                              << " defects:";

                    if (_qa->HasDefect("TotalOutlier"))
                        std::cout << " TotalOutlier(bit0)";

                    if (_qa->HasDefect("TerminalOutlier"))
                        std::cout << " TerminalOutlier(bit1)";

                    if (_qa->HasDefect("MarginalOutlier"))
                        std::cout << " MarginalOutlier(bit2)";

                    if (_qa->HasDefect("SectorLoss"))
                        std::cout << " SectorLoss(bit3)";

                    if (_qa->HasDefect("Misc"))
                        std::cout << " Misc(bit5)"
                                  << " comment=\"" << _qa->GetComment() << "\"";
                                        ...
                                        else
                                        {
                                                if (data->pid(part) != 11)
                                                {
...

                                                        {
                                                                ...
                                                                _hists->FillHists_pim_pid_with_cuts(data, event, part, *event->GetPims()[pim_idx]);
                                                        }
                                                }
                                        }
                                }
...
// ============================================================
// Cut level enum (used only for control flow)
// ============================================================
enum class CutLevel
{
        Loose,
...
inline const char *CutName(CutLevel lvl)
{
        switch (lvl)
        {
        case CutLevel::Loose:
...
// W/Q2 cuts
// ============================================================
inline bool PassesWQ2(const std::shared_ptr<Reaction> &evt)
{
        return (evt->W() > 1.35 && evt->W() <= 2.15 &&
                evt->Q2() > 1.95 && evt->Q2() <= 9.0);
}

// ============================================================
// QADB helper (DATA ONLY)
// ============================================================
inline bool PassesQA(const std::shared_ptr<QA::QADB> &qa,
                     const std::shared_ptr<Branches12> &data)
{
        if (!qa)
                return true;
...
// ============================================================
// Per-cut-level RECO processing (DATA)
// ============================================================
template <class CutType>
static void ProcessRecoCutLevel(
    CutLevel level,
    const std::shared_ptr<Branches12> &data,
    const std::shared_ptr<Histogram> &_hists,
    const std::shared_ptr<QA::QADB> &qa,
    double beam_energy,
    int &two_pion_possible,
    int &two_pion_selected)
{
        const char *cut = CutName(level);
        ...
        for (int part = 1; part < data->gpart(); ++part)
        {
                ...
                if (data->charge(part) > 0)
                {
                        ...
                        if (cuts->IsProton(part, cut))
                                event->SetProton(part);

                        if (cuts->IsPip(part, cut))
                                event->SetPip(part);
                }
                else
                {
        ...
        if (event->GetProtons().empty() || event->GetPips().empty())
                return;
        ...
        cuts->MarkCDFDOverlaps(data, cut,
                               drop_proton_idx,
                               drop_pip_idx,
                               _hists.get());
        ...
        for (size_t i = 0; i < event->GetProtons().size(); ++i)
        {
                for (size_t j = 0; j < event->GetPips().size(); ++j)
                {
                        int pidx = event->GetProtonIndices()[i];
                        int piidx = event->GetPipIndices()[j];

                        if (pidx == piidx)
                                continue;

                        if (drop_proton_idx.count(pidx))
                                continue;
                        if (drop_pip_idx.count(piidx))
                                continue;

                        two_pion_possible++;

                        event->CalcMissMassPim(*event->GetProtons()[i],
                                               *event->GetPips()[j]);
                        event->boost(*event->GetProtons()[i],
                                     *event->GetPips()[j]);
                        event->EffCorrFactor(*event->GetProtons()[i], *event->GetPips()[j]);
                        event->weight();
                        two_pion_selected++;

                        if (level == CutLevel::Loose)
                        {
                                _hists->Fill_MMSQ_mPim_2_comb(event);
                                _hists->Fill_histSevenD_prot_loose(event);
                                _hists->Fill_histSevenD_pip_loose(event);
...
// ============================================================
template <class CutType>
size_t run(std::shared_ptr<TChain> chain,
           const std::shared_ptr<Histogram> &_hists,
           const std::shared_ptr<QA::QADB> &qa,
           int thread_id)
{
        const size_t nEvents = chain->GetEntries();

        double beam_energy = 10.6041;
        if (getenv("CLAS12_E"))
                beam_energy = atof(getenv("CLAS12_E"));

        auto data = std::make_shared<Branches12>(chain, false);

        int two_pion_possible = 0;
        int two_pion_selected = 0;
        ...
        for (size_t ev = 0; ev < nEvents; ++ev)
        {
                ...
                if (thread_id == 0 && ev % 1000 == 0)
                        std::cout << "\r" << (100 * ev / nEvents) << " %" << std::flush;

                ProcessRecoCutLevel<CutType>(CutLevel::Loose,
                                             data, _hists, qa, beam_energy,
                                             two_pion_possible, two_pion_selected);

                ProcessRecoCutLevel<CutType>(CutLevel::Mid,
                                             data, _hists, qa, beam_energy,
                                             two_pion_possible, two_pion_selected);

                ProcessRecoCutLevel<CutType>(CutLevel::Tight,
                                             data, _hists, qa, beam_energy,
                                             two_pion_possible, two_pion_selected);
        }

        std::cout << "\nDone.\n";
        std::cout << "Two-pion possible: " << two_pion_possible << "\n";
        std::cout << "Two-pion selected: " << two_pion_selected << "\n";

        if (qa)
                std::cout << "Accumulated charge: "
